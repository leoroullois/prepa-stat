...
const parseFile = (fileName, inputPath) => {
	const input = fs
		.readFileSync(path.join(inputPath, fileName + ".tsv"), "utf8")
		.toString()
		.trim()
		.split("")
		.map((elt) => elt.replace(" ", "_"))
		.join("");
	return d3.tsvParse(input);
};
...
const exportFile = (file, fileName, outputPath) => {
	...
	fs.writeFile(path.join(outputPath, fileName + ".json"), myFile, (err) => {
		if (err) {
			console.log(err);
		} else {
...
const generateYear = (year) => {
	...
	const inputPath =
		"/media/leyo/DATA/Dev/Web/scei/lib/" +
		year +
		"/";
	const outputPath =
		"/media/leyo/DATA/Dev/Web/scei/src/assets/json/" +
		year +
		"/";
