            ...
            for j in i.find_all('td'):
                if n == 0:
                    if j.find('b') != None:
                        ...
                        tab.append(" ".join(re.split(r"\s+", str(j)[find_first_chevron(str(j))+1:find_second_chevron(str(j))].strip().replace('&amp;', '&').replace('\n', '').replace(" ","_").replace("Ã©","é").replace("Ã¢","â").replace("Ã¨","è"))))
                else:
